...
az webapp config container set --name $app_name --resource-group $resource_group_name --docker-custom-image-name codevault.azurecr.us/codeserver:v1 --docker-registry-server-url codevault.azurecr.us --docker-registry-server-user $APP_ID --docker-registry-server-password $SP_PASSWD --startup-file "yo"
az webapp config set --resource-group $resource_group_name --name $app_name --startup-file="docker run -d --name=codeserver -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -e DEFAULT_WORKSPACE=/config/workspace -p 443:443 -v config:/config --restart unless-stopped"
